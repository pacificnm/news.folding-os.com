...
Ported from chat.folding-os.com/backend/app/llm/agent.py, unchanged.

Event contract (consumed by app/api/chat.py -> SSE):
...
Cancellation: `cancel` (asyncio.Event) is checked before every round and
around every tool call. A hard task cancellation (client disconnect)
propagates out as asyncio.CancelledError; the endpoint persists whatever
text arrived.
"""
...
def tool_log_json(result: TurnResult) -> str:
    """Compact tool log for the assistant ChatMessage.toolCalls column."""
    return json.dumps(
        [
            {
                "name": e.name,
